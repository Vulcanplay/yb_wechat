...
export class HttpService {
  ...
  public requestNoLoading(url: string, options: RequestOptionsArgs): Observable<Response> {
  ...
  public requestNoHeader(url: string, options: RequestOptionsArgs): Observable<Response> {
    url = Utils.formatUrl(url.startsWith('http') ? url : APP_SERVE_URL + url);
    return Observable.create(observer => {
      this.nativeService.showLoading() ;
      console.log('%c 请求前 %c', 'color:blue', '', 'url', url, 'options', options);
  ...
  public getNoLoading(url: string, paramMap: any = null): Observable<any> {
    return this.requestNoLoading(url, new RequestOptions({
      method: RequestMethod.Get,
      search: HttpService.buildURLSearchParams(paramMap)
    }));
  }
  public getNoHeader(url: string, paramMap: any = null): Observable<any> {
    return this.requestNoHeader(url, new RequestOptions({
      method: RequestMethod.Get,
      search: HttpService.buildURLSearchParams(paramMap)
    }));
  }
  ...
  public posts(url: string, body: any = {}): Observable<Response> {
    return this.request(url, new RequestOptions({
      method: RequestMethod.Post,
      body: body,
      headers: new Headers({
  ...
  private requestFailed(url: string, options: RequestOptionsArgs, err: Response): void {
    ...
    if (!this.nativeService.getConnectionStatus()) {
