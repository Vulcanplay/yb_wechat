...
export class OfferPage {
  ...
  private ybOffer: any = {
    ...
    activeId: "",         //活动ID
    productTypeId: "",    //产品种类ID  B36B26FE-B930-4A29-8449-705760D49898
  ...
  pushYBOrder(){
    if (this.ybOffer.brandId === "" || this.ybOffer.carseriseId === ""){
      ...
      this.ewpService.getYBPrice(this.ybOffer).subscribe( r => {
        if(r.Code == 200){
          this.navCtrl.push(EWPYBOrderPage, {ybPrice: r.DataList.Table, ybOffer: this.ybOffer});
        } else {
  ...
  ybSelectBrand(){
    ...
    brandModal.onDidDismiss(data => {
      if (data){
        ...
        this.ybOffer.brandName = this.ybData.brandName;
        this.ybOffer.carseriseName = this.ybData.seriesName;
